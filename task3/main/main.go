...
func main() {
	...
	err := export(twtr, []string{"twtrdata.txt", "twtrdata.json"})
	err = export(fb, []string{"fbdata.txt", "fbdata.json"})
	err = export(lnkdin, []string{"lnkdin.txt"})
...
func export(u socialmedia.SocialMedia, filename []string) error {
	for _, ban := range (filename){
		f, err := os.OpenFile(ban, os.O_CREATE|os.O_WRONLY, 0755)
		if err != nil {
			return errors.New("an error occured opening the file: " + err.Error())
		}

		switch ban[len(ban)-3:] {
		case "txt":
		...
		case "son":
			...
			for abc, fd := range u.Feed() {
				...
				so, _ := json.Marshal(very)
				n, err := f.Write([]byte(string(so) + "\n"))
				if err != nil {
					return errors.New("an error occured writing to file: " + err.Error())
				}
				fmt.Printf("wrote %d bytes\n", n)
